...
describe('Events API', () => {
  ...
  describe('GET /api/events/:id', () => {
    it('應該回傳特定活動的詳細資訊', async () => {
      ...
      const response = await request(app).get('/api/events/event-123').expect(200);
    ...
    it('應該回傳 404 當活動不存在', async () => {
      ...
      const response = await request(app).get('/api/events/non-existent').expect(404);
  ...
  describe('GET /api/events/search', () => {
    it('應該根據查詢參數搜尋活動', async () => {
      ...
      const response = await request(app).get('/api/events/search?query=IU').expect(200);
    ...
    it('應該根據地點搜尋活動', async () => {
      ...
      const response = await request(app).get('/api/events/search?location=信義區').expect(200);
